...
public class NetWork
{
	...
	private String getResponseStr(Map<String, String> params, String url)
	{
		...
		try
		{
			...
			if (mDebug)
				Log.e(TAG, "enityStr" + responseCode);
			if (responseCode == 200)
			{
			...
			if (mDebug)
				Log.e(TAG, "enityStr" + EntityUtils.toString(httpResponse.getEntity()));
	...
	public void inint(final String userid, final String uactype, final String uregtype,
			final String ulogindev, final String upushtoken)
	{
	...
	public void updatePushtoken(final String sign, final String upushtoken)
	{
		...
		new Thread()
		{
			...
			public synchronized void run()
			{
				...
				String responseStr = getResponseStr(params, "users/updatePushtoken");
				...
				if (responseStr != null)
				{
					try
					{
						...
						if (code == 200)
						{
							msg.what = EventType.MSG_UPDAT_EPUSH_TOKEN_SUCCESS.ordinal();
						} else
						{
							msg.what = EventType.MSG_UPDAT_EPUSH_TOKEN_FAILED.ordinal();
						}
	...
	public void getRoomList(final String sign, final String pageNum, final String pageSize)
	{
		...
		new Thread()
		{
			...
			public synchronized void run()
			{
				...
				try
				{
					String responseStr = getResponseStr(params, "meeting/getRoomList");
					if (responseStr != null)
					{
						JSONObject jsonObject = new JSONObject(responseStr);
						...
						if (code == 200)
						{
							...
							{
								int meetusable = meetingListJson.getJSONObject(i).getInt(
										"meetusable");
								...
								int meettype2 = 1;
								int memnumber = meetingListJson.getJSONObject(i)
										.getInt("memnumber");
								String meetingid = meetingListJson.getJSONObject(i).getString(
										"meetingid");
								long jointime = meetingListJson.getJSONObject(i)
										.getLong("jointime");
								int owner = meetingListJson.getJSONObject(i).getInt("owner");
								String userid = meetingListJson.getJSONObject(i).getString(
										"meetinguserid");
								String meetname = meetingListJson.getJSONObject(i).getString(
										"meetname");
								String meetdesc = meetingListJson.getJSONObject(i).getString(
										"meetdesc");
								// String remain1 =
								...
								RoomItem roomItem = new RoomItem(null, meetusable, pushable,
										meettype1, meettype2, memnumber, meetingid, createRoomTime,
										owner, userid, meetname, meetdesc);
	 ...
	 * @param meetingid
	 */
	public void applyRoom(final String sign, final String meetingname, final String meetingtype,
			final String meetdesc, final String pushable)
	{
		new Thread()
		{
			@Override
	...
	public void updateRoomMinuxMemNumber(final String sign, final String meetingid)
	{
		new Thread()
		{
			...
			public synchronized void run()
			{
				...
				try
				{
					String ss = getResponseStr(params, "meeting/updateRoomMinuxMemNumber");
					if (mDebug)
						Log.e(TAG, "ss " + ss);
					if (ss != null)
					{
		...
		new Thread()
		{
			...
			public synchronized void run()
			{
				...
				try
				{
					...
					String ss = getResponseStr(params, "meeting/updateRoomAddMemNumber");
	...
	public void updateRoomEnable(final String sign, final String meetingid, final String enable)
	{
		new Thread()
		{
			...
			public synchronized void run()
			{
				...
				try
				{
					String ss = getResponseStr(params, "meeting/updateRoomEnable");
					if (mDebug)
						Log.e(TAG, "ss " + ss);
					if (ss != null)
					{
			...
			final String meetingMemNumber)
	{
		new Thread()
		{
			...
			public synchronized void run()
			{
				...
				try
				{
					String ss = getResponseStr(params, "meeting/updateRoomMemNumber");
					if (mDebug)
						Log.e(TAG, "ss " + ss);
					if (ss != null)
					{
	...
	public void updateRoomPushable(final String sign, final String meetingid, final String pushable)
	{
		new Thread()
		{
			...
			public synchronized void run()
			{
				...
				try
				{
					String ss = getResponseStr(params, "meeting/updateRoomPushable");
					if (mDebug)
						Log.e(TAG, "ss " + ss);
					if (ss != null)
					{
	...
	public void updateMeetRoomName(final String sign, final String meetingid, final String roomName)
	{
		new Thread()
		{
			...
			public synchronized void run()
			{
				...
				try
				{
					String ss = getResponseStr(params, "meeting/updateMeetRoomName");
					if (mDebug)
						Log.e(TAG, "ss " + ss);
					if (ss != null)
					{
	...
	public void getMeetingMsgList(final String sign, final String meetingid, final String pageNum,
		...
		new Thread()
		{
			...
			public synchronized void run()
			{
				...
				try
				{
					...
					String ss = getResponseStr(params, "meeting/getMeetingMsgList");
	 ...
	 * @param userid
	 */
	public void insertMeetingMsg(final String meetingid, final String messageid,
			final String messagetype, final String sessionid, final String strMsg,
			final String userid)
	{
		new Thread()
		{
			...
			public synchronized void run()
			{
				...
				try
				{
					...
					String ss = getResponseStr(params, "meeting/insertMeetingMsg");
	...
	public void insertSessionMeetingInfo(final String meetingid, final String sessionid,
			final String sessionstatus, final String sessiontype, final String sessionnumber)
	{
		new Thread()
		{
			@Override
			public synchronized void run()
			{
				...
				try
				{
					...
					String ss = getResponseStr(params, "meeting/insertSessionMeetingInfo");
	...
	public void updateSessionMeetingStatus(final String sessionid, final String sessionstatus)
	{
		new Thread()
		{
			...
			public synchronized void run()
			{
				...
				try
				{
					String ss = getResponseStr(params, "meeting/updateSessionMeetingStatus");
	...
	public void updateSessionMeetingEndtime(final String sessionid)
	{
		new Thread()
		{
			...
			public synchronized void run()
			{
				...
				try
				{
					String ss = getResponseStr(params, "meeting/updateSessionMeetingEndtime");
		...
		new Thread()
		{
			...
			public synchronized void run()
			{
				...
				try
				{
					...
					String ss = getResponseStr(params, "meeting/updateSessionMeetingNumber");
		...
		new Thread()
		{
			...
			public synchronized void run()
			{
				...
				try
				{
					...
					String ss = getResponseStr(params, "meeting/updateUserMeetingJointime");
					if (ss != null)
					{
						JSONObject jsonObject = new JSONObject(ss);
						Bundle bundle = new Bundle();
	...
	public void insertUserMeetingRoom(final String sign, final String meetingid)
	{
		new Thread()
		{
			...
			public synchronized void run()
			{
				...
				try
				{
					String ss = getResponseStr(params, "meeting/insertUserMeetingRoom");
					if (ss != null)
					{
						JSONObject jsonObject = new JSONObject(ss);
						Bundle bundle = new Bundle();
						...
						{
							msg.what = EventType.MSG_INSERT_USER_MEETING_ROOM_FAILED.ordinal();
						}
	 ...
	 * @param sessionnumber
	 */
	public void pushMeetingMsg(final String sign, final String meetingid, final String pushMsg,
			...
			public synchronized void run()
			{
				...
				try
				{
					...
					if (ss != null)
					{
						...
						if (code == 200)
						{
							msg.what = EventType.MSG_PUSH_MEETING_MSG_SUCCESS.ordinal();
						} else
						{
							msg.what = EventType.MSG_PUSH_MEETING_MSG_FAILED.ordinal();
						}
	 ...
	 * @param notification
	 */
	public void pushCommonMsg(final String sign, final List<String> targetid, final String pushMsg,
			final String notification)
	{
		new Thread()
		{
			...
			public synchronized void run()
			{
				...
				try
				{
					...
					if (ss != null)
					{
						...
						if (code == 200)
						{
							msg.what = EventType.MSG_PUSH_COMMO_NSG_SUCCESS.ordinal();
						} else
						{
							msg.what = EventType.MSG_PUSH_OMMO_NMSG_FAILED.ordinal();
						}
						bundle.putString("message", message);
						msg.setData(bundle);
						EventBus.getDefault().post(msg);
